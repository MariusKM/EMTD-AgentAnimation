...
Same as Infantry. Workflow per level: generate 4 → user picks keeper → clean up the RAW keeper (default ESRGAN 2× saved at 2K; NAFNet deblur on the ESRGAN-upscaled raw as fallback when the user reports ESRGAN issues — chain runs at 2K throughout) → composite the CLEANED RAW against prior composite → inspect QC → chain forward. Never skip steps. Never run the cleanup pass on the composite. Never chain off the raw keeper. Full reasoning: `../Archer/CLAUDE.md` § Part 1 § G2 + § L.
...
14. **ESRGAN 2× is the default cleanup pass; NAFNet deblur is the fallback; chain runs at 2K throughout** (LOCKED 2026-05-13 — same as Archer / Infantry; supersedes the earlier NAFNet-default at 1024). ESRGAN at `fal-ai/esrgan` with `scale: 2`, output saved at 2048×2048 as-is (no downscale). Switch to NAFNet deblur for a specific keeper only when the user flags the ESRGAN-cleaned composite as worse than raw; apply NAFNet to the ESRGAN-upscaled-to-2K raw, not the 1024 raw. Cavalry's `Refs/L1_Base.png` was already 2K (downscaled from `L1_Base_2K.png` to 1024 at scaffold time under the legacy 1K-chain rule); under the 2026-05-13 rule that downscale is no longer needed — restore `Refs/L1_Base.png` to the 2K source.
15. **Diff-mask composite is mandatory between every level lock and the next chain step**. **Cavalry-locked thresholds: `--low 15 --high 35`** (intensity-based, dim-invariant — deviates from Archer/Infantry default 10/25). **2K-era spatial defaults: `--pool 24 --mask-blur 6 --dilate 20`** (Cavalry-specific `--pool 24` doubled 2026-05-13 from the 1K-locked `--pool 12`; `mask-blur` and `dilate` follow the project-wide 2K defaults doubled from 1K `3 / 10`). The original Cavalry thresholds were locked at L2 (2026-05-06) after the 10/25 default produced visible face/helm soft-retouch leakage on the rider's portrait — the model's subtle face refresh on every edit pushes 5-15-unit pixel diffs through the 10/25 mask. Bumping low 10→15 puts that drift below the preserve floor and bumping pool 8→12 (now 16→24 at 2K) smooths small isolated diff regions. The 1K-era L2 defaults reduced `transition_pct` from 14.88% → 2.95%, `preserved_drift` from 0.105 → 0.056, and `mask_islands` from 11 → 5 with `edit_fidelity` only marginally affected (0.266 → 0.345, well within target). Tier-break dilate bump 30-40 (was 15-20 at 1K) still applies. ⚠ The 2K spatial doubling is a geometric scaling — these Cavalry-specific reductions in `transition_pct` / `preserved_drift` / `mask_islands` were measured at 1K and will need to be re-validated on the first 2K keeper; retune the spatial knobs if QC drifts.
